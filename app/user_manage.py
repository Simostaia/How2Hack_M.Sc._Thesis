...
def manage(request):
    ...
    if POST_VALUES["action"] == "update_infos":
        ...
        if len(POST_VALUES["professione"]) >=3 \
        and len(POST_VALUES["nome"]) >=3 \
        and len(POST_VALUES["ssh_pub_user"]) >=3 \
        and len(POST_VALUES["cognome"]) >=3: 
            me.professione = POST_VALUES["professione"]
            me.nome = POST_VALUES["nome"]
            me.cognome = POST_VALUES["cognome"]
            me.ssh_pub_user = POST_VALUES["ssh_pub_user"]
            me.save()
